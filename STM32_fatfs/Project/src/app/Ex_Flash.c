...
uint8 Sec_Data[Ex_FLASH_SecSize];

/* 移植接口——start */
#define	    Ex_Flash_CS_Low()		GPIO_ResetBits(GPIOA,GPIO_Pin_4);
#define     Ex_Flash_CS_High()          GPIO_SetBits(GPIOA,GPIO_Pin_4);

uint8 MCU_ReadWriteByte(uint8 TxData)
{
...
uint8 Ex_FLASH_SectorErase(uint32 Sec_Num)
{
    uint32 SectorAddr = 0;
    uint32 i = 0;
...
uint8 Ex_Flash_RD_Sec(uint32 Sec_Num,uint8* RcvBuf)
{
  uint32 addr = 0;
  uint32 i;
  ...
  for (i = 0; i < Ex_FLASH_SecSize; i++)		
  {
    RcvBuf[i] = MCU_ReadWriteByte(0xFF);		
  }
  Ex_Flash_CS_High();			
  return (Ex_Flash_OK);
}

uint8 Ex_Flash_RD(uint32 ReadAddr,uint8* RcvBuf,uint32 NByte)
{
  uint32 i = 0;
  ...
  * @retval None    
  */
uint8 Ex_Flash_PageWrite(uint32 WriteAddr,uint8* pBuffer,  uint16 NumByteToWrite)
{
...
/**
  ...
  * @retval None    
  */
uint8 Ex_FLASH_BufferWrite(uint32_t WriteAddr,uint8_t* pBuffer,  uint16_t NumByteToWrite)
{
...
/**
  ...
  * @retval None    
  */
uint8 Ex_FLASH_Write(uint32_t WriteAddr,uint8_t* pBuffer,  uint16_t NumByteToWrite)
{
    //uint32_t Start_Sec;
    uint32_t Start_Addr;
    uint32_t Sec_num;
